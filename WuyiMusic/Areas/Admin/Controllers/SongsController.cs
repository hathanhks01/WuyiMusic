...
namespace WuyiMusic.Areas.Admin.Controllers
{
    ...
    public class SongsController : Controller
    {
        ...
        {
            string url = $"https://localhost:7178/api/Songs/get-by-id?id={id}";
        ...
        public async Task<IActionResult> Create([Bind("SongId,ArtistId,AlbumId,GenreId,Title,Duration,ImgUrl,UploaddAt")] Song song, IFormFile songFile)
        {
            { 
                try
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, [Bind("SongId,ArtistId,AlbumId,GenreId,Title,Duration,FilePath,ImgUrl,UploadAt")] Song song)
        {
            if (id != song.SongId)
            {
                return NotFound();
